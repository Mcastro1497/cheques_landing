import Calculator from "@/components/Calculator";
import Faq from "@/components/Faq";
import { LbLogo } from "@/components/Logo";

const WHATSAPP = "https://wa.me/5493413481637";
const AGENDA = "https://meetings.hubspot.com/manuelcastro/asesoria-lb-finanzas";

const VALORES = [
  { titulo: "Tasas competitivas", detalle: "Mejores condiciones que las alternativas tradicionales." },
  { titulo: "Acreditación en ~48 hs", detalle: "Convertí tus echeqs en pesos rápido." },
  { titulo: "100% digital", detalle: "Gestionás todo online, sin trámites de banco." },
];

function Check() {
  return (
    <svg viewBox="0 0 20 20" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden="true">
      <path d="M4 10.5l4 4 8-9" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

function Whatsapp({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="currentColor" aria-hidden="true">
      <path d="M.057 24l1.687-6.163a11.867 11.867 0 01-1.587-5.946C.16 5.335 5.495 0 12.05 0a11.82 11.82 0 018.413 3.488 11.82 11.82 0 013.48 8.414c-.003 6.557-5.338 11.892-11.893 11.892a11.9 11.9 0 01-5.688-1.448L.057 24zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884a9.86 9.86 0 001.51 5.26l-.999 3.648 3.978-1.607zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.5-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z" />
    </svg>
  );
}

export default function Home() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-10 sm:py-14">
      {/* Hero */}
      <header className="mb-8">
        <LbLogo className="h-8 w-auto text-violeta" />
        <h1 className="mt-6 text-3xl font-bold tracking-tight text-tinta sm:text-4xl">
          Financiamiento ágil para PYMES
        </h1>
        <p className="mt-3 max-w-xl text-base text-slate-600 sm:text-lg">
          Convertí tus echeqs en pesos y accedé a mejores condiciones de
          financiamiento sin depender únicamente del banco.
        </p>
      </header>

      {/* Propuestas de valor */}
      <ul className="mb-8 grid gap-3 sm:grid-cols-3">
        {VALORES.map((v) => (
          <li
            key={v.titulo}
            className="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-200"
          >
            <div className="flex items-center gap-2">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violeta text-white">
                <Check />
              </span>
              <span className="text-sm font-semibold text-tinta">{v.titulo}</span>
            </div>
            <p className="mt-1.5 text-xs leading-relaxed text-slate-500">{v.detalle}</p>
          </li>
        ))}
      </ul>

      <Calculator />

      <Faq />

      {/* CTAs */}
      <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
        <a
          href={AGENDA}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-violeta px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-violeta-comp sm:w-auto"
        >
          Agendá una reunión
        </a>
        <a
          href={WHATSAPP}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex w-full items-center justify-center gap-2 rounded-xl border border-violeta px-6 py-3 text-sm font-semibold text-violeta transition hover:bg-lavanda sm:w-auto"
        >
          <Whatsapp className="h-5 w-5" />
          Hablá con un asesor
        </a>
      </div>
    </main>
  );
}
